...
@Component({
  ...
  templateUrl: './entertainment-news.html',
  styleUrl: './entertainment-news.css'
})
export class EntertainmentNews {
  ...
  load(page: number) {
     this.newsService.getByCategory('entertainment', page, this.limit).subscribe((res: PagedResponse<News>) => {
      console.log(res);
      this.news = res.items;
      this.page = res.page;
