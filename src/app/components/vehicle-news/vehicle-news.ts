...
@Component({
  ...
  templateUrl: './vehicle-news.html',
  styleUrl: './vehicle-news.css'
})
export class VehicleNews {
  news: News[] = [];
  page = 1;
  totalPages = 1;
  limit = 12;
  ...
  load(page: number) {
     this.newsService.getByCategory('auto', page, this.limit).subscribe((res: PagedResponse<News>) => {
      console.log(res);
      this.news = res.items;
      this.page = res.page;
